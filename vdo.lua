...
Library = {
	...
	MainColor = Color3.fromRGB(0, 21, 21);
	BackgroundColor = Color3.fromRGB(0, 11, 14);
...
Library = {
	...
	RegistryMap = {};
	HudRegistry = {};
	FontColor = getgenv().ColorUI.Font;
	MainColor = getgenv().ColorUI.Main;
	BackgroundColor = getgenv().ColorUI.Back;
	AccentColor = getgenv().ColorUI.Accent;
	InlineColor = getgenv().ColorUI.Inline;
	OutlineColor = getgenv().ColorUI.Outline;
	OpenedFrames = {};
};
...
function Library:GetTextBounds(Text, Font, Size)
	return TextService:GetTextSize(Text, Size, Font, Vector2.new(1920, 1080)).X,TextService:GetTextSize(Text, Size, Font, Vector2.new(1920, 1080)).Y
end;
...
do
	...
	function Funcs:Video()
		video = {}
		local Groupbox = self;
		local Container = Groupbox.Container;
		local x=Instance.new("VideoFrame")
		x.Parent = Container
		x.AnchorPoint = Vector2.new(0.5, 0.5)
		x.Size = UDim2.new(1,0,0,275)
		x.ZIndex = 5;
		Groupbox:AddBlank(5);
		Groupbox:Resize();
		...
		function SaveVideo(id)
			if not isfile("videos/" .. id .. ".webm") then
				local Video = Request(id)
				if Video then
					writefile("videos/" .. id .. ".webm", Video)
				else
				end
			end
		end
		
		function video:SetValue(xd)
			if xd == true then
				x:Play()
			elseif xd == false then
				x:Pause()
			end
		end
		function video:SetLoop(xd)
			x.Looped = xd
		end
		function video:ChangeVideo(xd)
			SaveVideo(xd)
			Library:Notify("Wait Loading...")
			x.Video = getsynasset(xd)
		end
		function video:ChangeVideo2(xd)
			SaveVideo(xd)
			Library:Notify("Wait Loading...")
			x.Video = getsynasset("videos/" .. xd .. ".webm")
		end
		return video
	end
...
function Library:CreateWindow(WindowTitle)
	...
	function Window:SetVisible(boolean)
		pcall(function()
			Outer.Visible = not boolean;
			ModalElement.Modal = boolean;
		end)
	end;
