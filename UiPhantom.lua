local menu
local MenuName = isfile("zeromonth/menuname.txt") and readfile("zeromonth/menuname.txt") or nil
...
local function GetConfigs()
	...
	if #result <= 0 then
		writefile("zeromonth/" .. menu.game .. "/Default.zm", "")
	end
	return result
end
...
local function UnpackRelations()
	local str = isfile("zeromonth/relations.zm") and readfile("zeromonth/relations.zm") or nil
	local final = {
		friends = {},
		priority = {},
	}
	if str then
		if str:find("zm:{{") then
			writefile("zeromonth/relations.zm", "friends:\npriority:")
			return
		end
...
local function WriteRelations()
	...
	writefile("zeromonth/relations.zm", str)
end
CreateThread(function()
...
function menu.Initialize(menutable)
	...
	do
		...
		Draw:MenuBigText(MenuName or "ZeroMonth", true, false, 6, 6, bbmenu)
	...
	local function SaveCurSettings() --ANCHOR figgies
		local figgy = "ZeroMonth \nmade with <3 by phantom4k and f1zyshka\n\n" -- screw zarzel XD (and json and classy)
	...
	function menu.saveconfig()
		...
		CreateNotification('Saved "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.zm"!')
		UpdateConfigs()
	end
	
	function menu.loadconfig()
		local configname = "zeromonth/"
			.. menu.game
			.. "/"
			.. menu.options["Settings"]["Configuration"]["ConfigName"][1]
			.. ".zm"
	...
	local function buttonpressed(bp)
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
			...
			CreateNotification('Deleted "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.zm"!')
			UpdateConfigs()
		...
		if menu then
			...
			if menu.open then
				if menu.tabnames[menu.activetab] == "Settings" then
					bbmenu[27].Text = menu:GetVal("Settings", "Cheat Settings", "Custom Menu Name") and menu:GetVal("Settings", "Cheat Settings", "MenuName") or "ZeroMonth"
	...
	function menu:unload()
		...
		game:service("ContextActionService"):UnbindAction("ZM Keycheck")
